...
- Google returns the OpenID `email` scope in its canonical `userinfo.email` form. `run_installed_app_flow` therefore relaxes oauthlib's verbatim scope-equality check for the duration of the token exchange, restores the previous environment value, and verifies scope equivalence itself through `canonical_scope_set`, raising `GoogleOAuthScopeMismatchError` when the granted set is not the approved set. Without this the bootstrap failed after successful consent with a bare oauthlib `Warning`.
